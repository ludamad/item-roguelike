...
export class Condition {
  ...
  public getName() {
    return this._noDisplay
      ...
      : this.name
      ? this.name
      : Condition.condNames[this._type];
  }
...
export class HealthVariationCondition extends Condition {
  ...
  constructor(def: IConditionDef) {
    ...
    if (def && def.amount) {
      this.hpPerTurn =
        def.nbTurns === 0 ? def.amount : def.amount / def.nbTurns;
    }
  }
...
export class FrozenCondition extends Condition {
  ...
  public update(owner: Actor): boolean {
    ...
    owner.col = Core.ColorUtils.add(
      Core.ColorUtils.multiply(FROST_COLOR, progress),
