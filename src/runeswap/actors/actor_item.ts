...
export class Container implements IActorFeature {
  ...
  public getQualifiedName(owner: Actor, count: number = 1): string {
    ...
    if (maxCount > 0.75 * this.actorIds.length && bestType) {
      return (
        this.getCapacityQualifier() +
        owner.getname(count) +
        " of " +
        ActorFactory.getActorTypeName(bestType, 2)
      );
    }
    return this.getCapacityQualifier() + owner.getname(count);
  }
  ...
  public shouldAutoEquip(owner: Actor, item: Actor): boolean {
    ...
    for (
      let i: number = 0, count: number = itemSlots.length;
      i < count && !isSlotAvailable;
      ++i
    ) {
  ...
  public isSlotEmpty(slot: string): boolean {
    ...
    if (slot === SLOT_RIGHT_HAND || slot === SLOT_LEFT_HAND) {
      ...
      if (
        this.getFromSlot(SLOT_LEFT_HAND) ||
        this.getFromSlot(SLOT_RIGHT_HAND)
      ) {
        return false;
      }
    }
...
export class Pickable implements IActorFeature {
  ...
  get onThrowEffect() {
    return this.onThrowEffector ? this.onThrowEffector.effect : undefined;
  }
  ...
  public setOnUseEffect(
    effect: IEffect,
    targetSelector: TargetSelector,
    message?: string,
    destroyOnEffect: boolean = false
  ) {
    this.onUseEffector = new Effector(
      effect,
      targetSelector,
      message,
      destroyOnEffect
  ...
  public setOnThrowEffect(
    effect: IEffect,
    targetSelector: TargetSelector,
    message?: string,
    destroyOnEffect: boolean = false
  ) {
    this.onThrowEffector = new Effector(
      effect,
      targetSelector,
      message,
      destroyOnEffect
  ...
  public pick(
    ...
    if (
      ...
      wearer.container &&
      wearer.container.add(owner, wearer)
    ) {
      ...
      if (!fromEquip) {
        if (
          owner.equipment &&
          wearer.isPlayer() &&
          wearer.container.shouldAutoEquip(wearer, owner)
        ) {
...
export class Equipment implements IActorFeature {
  ...
  public equipOnSlot(owner: Actor, wearer: Actor, slot: string) {
    ...
    if (wearer === Actor.specialActors[SpecialActorsEnum.PLAYER]) {
      Umbra.logger.warn(
        transformMessage(
          "[The actor1] equip[s] [the actor2] on [its] " + slot,
          wearer,
          owner
        )
      );
  ...
  public equip(owner: Actor, wearer: Actor) {
    if (!this.slots) {
      Umbra.logger.error(
        transformMessage("Error! Cannot equip [the actor1]", owner)
      );
      return;
    }
  ...
  public unequip(owner: Actor, wearer: Actor, beforeDrop: boolean = false) {
    ...
    if (
      slot &&
      !beforeDrop &&
      wearer === Actor.specialActors[SpecialActorsEnum.PLAYER]
    ) {
      Umbra.logger.warn(
        transformMessage(
          "[The actor1] unequip[s] [the actor2] from [its] " + slot,
          wearer,
          owner
        )
      );
...
export class Magic implements IActorFeature {
  ...
  public async zap(owner: Actor, wearer: Actor): Promise<boolean> {
    if (this._charges === 0) {
      Umbra.logger.info(
        transformMessage(
          "[The actor1's] " + owner.name + " is uncharged",
          wearer
        )
      );
      return false;
  ...
  public init(def: IActivableDef) {
    if (def) {
      ...
      if (def.onActivateEffector) {
        this.onActivateEffector = ActorFactory.createEffector(
          def.onActivateEffector
        );
      }
  ...
  public activate(owner: Actor, activator?: Actor): boolean {
    if (owner.lock && owner.lock.isLocked()) {
      ...
      if (
        activator &&
        activator.container &&
        activator.container.contains(keyId, true)
      ) {
        ...
        Umbra.logger.info(
          transformMessage(
            "[The actor1] unlock[s] [the actor2].",
            activator,
            owner
          )
        );
        ...
        Umbra.logger.info(
          transformMessage("[The actor1] is locked and needs a key.", owner)
        );
        return false;
      }
    }
...
export class Lever extends Activable {
  ...
  public activate(_owner: Actor, activator?: Actor): boolean {
    let mechanism =
      this.actorId !== undefined ? Actor.fromId(this.actorId) : undefined;
    return mechanism && mechanism.activable
      ? mechanism.activable.activate(mechanism, activator)
      : false;
  }
  ...
  public deactivate(_owner: Actor, _activator?: Actor): boolean {
    let mechanism =
      this.actorId !== undefined ? Actor.fromId(this.actorId) : undefined;
    return mechanism && mechanism.activable
      ? mechanism.activable.deactivate(mechanism)
      : false;
  }
  ...
  public switchLever(_owner: Actor, activator?: Actor): boolean {
    let mechanism =
      this.actorId !== undefined ? Actor.fromId(this.actorId) : undefined;
    return mechanism && mechanism.activable
      ? mechanism.activable.switchLever(mechanism, activator)
      : false;
  }
}
