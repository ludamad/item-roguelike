...
export class ActorFactory {
  ...
  public static computeLevelProbabilities(
    probabilities: IProbabilityMap,
    level: number
  ): { [index: string]: number } {
    ...
    for (let itemProb of probabilities.classProb) {
      ...
      if (typeof itemProb.prob === "number") {
        ...
        ret[itemProb.clazz] = ActorFactory.getValueForLevel(
          <number[][]>itemProb.prob,
          level
        );
      }
    }
  ...
  public static createEffector(def: IEffectorDef) {
    return new Effector(
      ActorFactory.createEffect(def.effect),
      ActorFactory.createTargetSelector(def.targetSelector),
      def.message,
      def.destroyOnEffect
    );
  }
  ...
  public static load(persister: Yendor.IPersister): Promise<void> {
    return new Promise<void>((resolve) => {
      persister.loadFromKey(PERSISTENCE_ACTORS_SEQ_KEY).then((value) => {
        ActorFactory.seq = value;
        resolve();
      });
    });
  }
  ...
  private static createActor(
    ...
    if (!actor.charCode) {
      Umbra.logger.critical(
        "ERROR : no character defined for actor type " + def.name
      );
    }
    return actor;
  }

  private static populateActor(
    actor: Actor,
    def: IActorDef,
    tilePicker?: ITilePicker,
    inventoryPicker?: IInventoryItemPicker,
    ...
    if (def.prototypes) {
      for (let className of def.prototypes) {
        if (ActorFactory.actorDefs[className]) {
          ActorFactory.populateActor(
            actor,
            ActorFactory.actorDefs[className],
            tilePicker,
            inventoryPicker,
            lootHandler
          );
    ...
    if (def.ai) {
      actor.ai = ActorFactory.createAi(
        actor,
        def.ai,
        tilePicker,
        inventoryPicker,
    ...
    if (def.destructible) {
      actor.destructible = new Destructible(
        def.destructible,
        actor.destructible
      );
    }
