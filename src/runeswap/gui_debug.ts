...
export class DebugMenu extends Gui.Widget {
  constructor() {
    ...
    vpanel.addChild(
      this.mapRenderModeButton(
        "dbg map transp",
        Map.MapRenderModeEnum.TRANSPARENCY
      )
    );
    vpanel.addChild(
      this.mapRenderModeButton("dbg map light", Map.MapRenderModeEnum.LIGHTMAP)
    );
    vpanel.addChild(
      this.mapRenderModeButton("dbg map fov", Map.MapRenderModeEnum.FOV)
    );
    vpanel.addChild(
      this.mapRenderModeButton(
        "dbg map all",
        Map.MapRenderModeEnum.ALL_SEEING_EYE
      )
    );
    vpanel.addChild(
      this.mapRenderModeButton("dbg map normal", Map.MapRenderModeEnum.NORMAL)
    );
    vpanel.addChild(
      new Gui.Button({
        autoHideWidget: this,
  ...
  private mapRenderModeButton(
    label: string,
    mode: Map.MapRenderModeEnum
  ): Gui.Button {
