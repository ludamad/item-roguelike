...
export class Application {
  ...
  public run(
    startingScene: Scene,
    options: IApplicationOptions = Application.DEFAULT_OPTIONS
  ): void {
    $(window).focus(this.onGainFocus.bind(this));
    $(window).blur(this.onLoseFocus.bind(this));
    this.options = <IApplicationOptions>{
      backgroundAnimation:
        options.backgroundAnimation || Constants.DEFAULT_BACKGROUND_ANIMATION,
      consoleHeight: options.consoleHeight
        ? options.consoleHeight
        : Constants.DEFAULT_CONSOLE_HEIGHT,
      consoleWidth: options.consoleWidth
        ? options.consoleWidth
        : Constants.DEFAULT_CONSOLE_WIDTH,
      defaultBackgroundColor:
        options.defaultBackgroundColor || Constants.DEFAULT_BACKGROUND_COLOR,
      defaultForegroundColor:
        options.defaultForegroundColor || Constants.DEFAULT_FOREGROUND_COLOR,
      fontFileName: options.fontFileName
        ? options.fontFileName
        : Constants.DEFAULT_FONT_FILE_NAME,
      renderDivSelector: options.renderDivSelector
        ? options.renderDivSelector
        : Constants.DEFAULT_DIV_SELECTOR,
      ticksPerSecond: options.ticksPerSecond
        ? options.ticksPerSecond
        : Constants.DEFAULT_TICKS_PER_SECOND,
    };
    if (Yendor.urlParams[Constants.URL_PARAM_FONT_FILENAME]) {
      this.options.fontFileName =
        Yendor.urlParams[Constants.URL_PARAM_FONT_FILENAME];
    }
    this.dirtyFrame = true;
    this.tickLength = 1.0 / (this.options.ticksPerSecond || 1);
