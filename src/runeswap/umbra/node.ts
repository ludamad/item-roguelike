...
export class Node extends Core.TreeNode {
  ...
  public logSceneGraph(pad: string = "", withStats: boolean = true) {
    if (withStats) {
      console.log(
        "##### Umbra node stats :" +
          this.computeNodeCount(true) +
          " nodes (" +
          this.computeNodeCount(false) +
          " visibles) #####"
      );
    }
  ...
  public expand(w: number, h: number) {
    ...
    if (
      ...
      (this._expand === ExpandEnum.HORIZONTAL ||
    ...
    if (
      ...
      (this._expand === ExpandEnum.VERTICAL || this._expand === ExpandEnum.BOTH)
    ) {
  ...
  public isVisible(): boolean {
    return (
      this.visible &&
      (this.__parent === undefined || (<Node>this.__parent).isVisible())
    );
  }
  ...
  public removeChild<T extends Core.TreeNode>(node: T): T {
  ...
  public center() {
    ...
    this.moveTo(
      Math.floor((parentBox.w - this.boundingBox.w) / 2),
  ...
  public computeBoundingBox() {
    ...
    for (let child of this.children) {
      ...
      if (node.visible) {
        ...
        this.boundingBox.w = Math.max(
          this.boundingBox.w,
          node.boundingBox.x + node.boundingBox.w
        );
        this.boundingBox.h = Math.max(
          this.boundingBox.h,
          node.boundingBox.y + node.boundingBox.h
        );
      }
    }
  }
