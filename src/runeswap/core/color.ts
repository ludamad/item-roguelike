...
export class ColorUtils {
  ...
  public static add(col1: Color, col2: Color): Color {
    let r =
      (((<number>col1) & 0xff0000) >> 16) + (((<number>col2) & 0xff0000) >> 16);
    let g =
      (((<number>col1) & 0x00ff00) >> 8) + (((<number>col2) & 0x00ff00) >> 8);
  ...
  private static toRgbFromString(color: String): number[] {
    ...
    if (color.charAt(0) === "#") {
      ...
      return [
        parseInt(rgbList[0], 10),
        parseInt(rgbList[1], 10),
        parseInt(rgbList[2], 10),
      ];
    }
    return [0, 0, 0];
  }
