...
export class Fov {
  ...
  private computeQuadrantVertical(
    inFov: boolean[][],
    xPov: number,
    yPov: number,
    maxRadius: number,
    ...
    while (!done && y >= 0 && y < this.height) {
      ...
      let processedCell: number = Math.floor(
        (minAngle + halfSlope) / slopePerCell
      );
      let xMin: number = Math.max(0, xPov - iteration);
      let xMax: number = Math.min(this.width - 1, xPov + iteration);
      done = true;
      for (
        let x: number = xPov + processedCell * dx;
        x >= xMin && x <= xMax;
        x += dx
      ) {
        ...
        if (lastLineObstacleCount > 0 && !inFov[x][y]) {
          ...
          if (
            visible &&
            !this.canSee(inFov, x, y - dy) &&
            x - dx >= 0 &&
            x - dx < this.width &&
            !this.canSee(inFov, x - dx, y - dy)
          ) {
            visible = false;
          } else {
            while (visible && idx < lastLineObstacleCount) {
              if (
                this.startAngle[idx] > endSlope ||
                this.endAngle[idx] < startSlope
              ) {
                ++idx;
              } else {
                if (this.isTransparent(x, y)) {
                  if (
                    centreSlope > this.startAngle[idx] &&
                    centreSlope < this.endAngle[idx]
                  ) {
                    visible = false;
                  }
                } else {
                  if (
                    startSlope >= this.startAngle[idx] &&
                    endSlope <= this.endAngle[idx]
                  ) {
                    visible = false;
                  } else {
                    this.startAngle[idx] = Math.min(
                      this.startAngle[idx],
                      startSlope
                    );
  ...
  private computeQuadrantHorizontal(
    inFov: boolean[][],
    xPov: number,
    yPov: number,
    maxRadius: number,
    ...
    while (!done && x >= 0 && x < this.width) {
      ...
      for (
        let y: number = yPov + processedCell * dy;
        y >= yMin && y <= yMax;
        y += dy
      ) {
        let centreSlope: number = processedCell * slopePerCell;
