...
export class PathFinder {
  ...
  constructor(
    ...
    this.walkCostFunction = walkCostFunction
      ? walkCostFunction
      : function (_from: Core.Position, _to: Core.Position) {
          return 1;
        };
    this.heuristicFunction = heuristicFunction
      ? heuristicFunction
      : allowDiagonals
      ? Core.Position.distance
  ...
  public getPath(
    ...
    while (openSet.size() > 0 && step > 0) {
      ...
      let adjacentCells: Core.Position[] = current.getAdjacentCells(
        this.mapWidth,
        this.mapHeight,
        this.allowDiagonals
      );
      for (let neighbor of adjacentCells) {
        if (!this.arrayContains(closedSet, neighbor)) {
          ...
          if (cost > 0) {
            ...
            if (
              !neighborInOpenSet ||
              tentativeGScore < gScore[neighborOffset]
            ) {
              cameFrom[neighborOffset] = current;
              gScore[neighborOffset] = tentativeGScore;
              fScore[neighborOffset] =
                tentativeGScore + this.heuristicFunction(neighbor, to);
  ...
  private reconstructPath(
    cameFrom: { [index: number]: Core.Position },
