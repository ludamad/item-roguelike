...
export abstract class Random {
   ...
   */
  public abstract getNumber(min: number, max: number): number;
   ...
   */
  public getRandomChance(chances: { [index: number]: number }): string | number;
  public getRandomChance(chances: {
    [index: string]: number;
  }): string | number {
...
export class CMWCRandom extends Random {
  private static DIVIDER: number = 1 / Math.pow(2, 32);
