  ...
  if (fixBorders) {
    for (let cx: number = x; cx < x + w; ++cx) {
      if (
        ...
        con.text[cx][y] === Yendor.CHAR_TEEE ||
        con.text[cx][y] === Yendor.CHAR_TEEW ||
  ...
  if (fixBorders) {
    for (let cy: number = y; cy < y + h; ++cy) {
      if (
        ...
        con.text[x][cy] === Yendor.CHAR_TEEN ||
        con.text[x][cy] === Yendor.CHAR_TEES ||
...
export function checkPos(x: number, y: number): boolean {
  return (
    x >= 0 &&
    y >= 0 &&
    x < Umbra.application.getConsole().width &&
    y < Umbra.application.getConsole().height
  );
}
