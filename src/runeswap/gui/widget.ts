...
export class Widget extends Umbra.Node implements Umbra.IEventListener {
  public static getActiveModal(): Widget | undefined {
    return Widget.activeModal;
  }
  ...
  public isModal(): boolean {
    return (
      ...
      (<Widget>this.__parent).isModal === undefined
        ? false
        : (<Widget>this.__parent).isModal())
    );
  }
  ...
  public isActive(): boolean {
    return (
      ...
      (this.__parent === undefined ||
      (<Widget>this.__parent).isActive === undefined
        ? false
        : (<Widget>this.__parent).isActive())
    );
  }
  ...
  protected showOnEventType(eventType: string, callback?: (data: any) => void) {
    ...
    (<any>this)[Umbra.EventManager.eventTypeToCallback(eventType)] = function (
      data: any
    ) {
      this.show();
      if (callback) {
...
export class VSeparator extends Widget {
  ...
  public onRender(con: Yendor.Console) {
    ...
    con.clearFore(
      getConfiguration().color.foregroundDisabled,
      pos.x,
      pos.y,
      1,
      h
    ...
    if (
      pos.y + h < Umbra.application.getConsole().height - 1 &&
  ...
  public onRender(con: Yendor.Console) {
    ...
    if (this.label) {
      ...
      con.print(
        ...
        pos.y,
        this.label,
        getConfiguration().color.titleForeground
      );
    }
  }
...
export class Draggable
  extends OptionWidget<IDragOption>
  ...
  public onStartDrag(_event: Umbra.IDragEvent) {
    if (
      ...
      Umbra.resetInput();
      let callback:
        | ((dropped: Draggable, data: any) => void)
        | undefined = this.getOptions().startDragCallback;
      if (callback) {
  ...
  public onEndDrag(_event: Umbra.IDragEvent) {
    if (this === Draggable.drag) {
      ...
      let callback:
        | ((dropped: Draggable, data: any) => void)
        | undefined = this.getOptions().endDragCallback;
      if (callback) {
        callback(this, this.getOptions().data);
      }
  ...
  public onUpdate(_time: number) {
    if (Draggable.drag === this) {
      ...
      Draggable.dragPos.x =
        mousePos.x + this.boundingBox.x - Draggable.startDrag.x;
      Draggable.dragPos.y =
        mousePos.y + this.boundingBox.y - Draggable.startDrag.y;
      let maxw: number =
        Umbra.application.getConsole().width - this.boundingBox.w;
      let maxh: number =
        Umbra.application.getConsole().height - this.boundingBox.h;
      let fatherPos = new Core.Position(
        -this.boundingBox.x,
        -this.boundingBox.y
      );
      this.local2Abs(fatherPos);
      Draggable.dragPos.x = Core.clamp(
        Draggable.dragPos.x,
        -fatherPos.x,
        maxw - fatherPos.x
      );
      Draggable.dragPos.y = Core.clamp(
        Draggable.dragPos.y,
        -fatherPos.y,
        maxh - fatherPos.y
      );
      if (
        this.options.minDragX !== undefined &&
        Draggable.dragPos.x < this.options.minDragX
      ) {
        Draggable.dragPos.x = this.options.minDragX;
      } else if (
        this.options.maxDragX !== undefined &&
        Draggable.dragPos.x > this.options.maxDragX
      ) {
        Draggable.dragPos.x = this.options.maxDragX;
      }
      if (
        this.options.minDragY !== undefined &&
        Draggable.dragPos.y < this.options.minDragY
      ) {
        Draggable.dragPos.y = this.options.minDragY;
      } else if (
        this.options.maxDragY !== undefined &&
...
export class DragTarget
  extends OptionWidget<IDropOption>
  implements Umbra.IEventListener {
  ...
  public onPostRender(con: Yendor.Console) {
    if (Draggable.isDragging() && this.isActive()) {
      ...
      (<Widget>this.__parent).local2Abs(pos);
      con.clearBack(
        getConfiguration().color.backgroundActive,
        pos.x,
        pos.y,
        this.boundingBox.w,
        this.boundingBox.h
...
export class Button extends Label<IButtonOption> {
  ...
  public onRender(con: Yendor.Console) {
    ...
    this.renderWithColor(
      ...
      pos,
      this.active
        ? getConfiguration().color.foregroundActive
        : getConfiguration().color.foreground,
      this.active
        ? getConfiguration().color.backgroundActive
        : getConfiguration().color.background
    );
  ...
  public onUpdate(_time: number) {
    ...
    this.pressed =
      this.active && Umbra.wasMouseButtonReleased(Umbra.MouseButtonEnum.LEFT);
    if (
      this.pressed ||
      (this.options.asciiShortcut &&
        Umbra.wasCharPressed(this.options.asciiShortcut))
    ) {
      Umbra.resetInput();
...
export class Slider extends OptionWidget<ISliderOption> {
  ...
  public setValue(value: number) {
    this.value = value;
    this.value = Core.clamp(
      this.value,
      this.options.minValue,
      this.options.maxValue
    );
    let dragPosX: number = Math.floor(
      ((this.boundingBox.w - 1) * (this.value - this.options.minValue)) /
  ...
  public onInit() {
    this.dragButton = new Button({
      expand: Umbra.ExpandEnum.NONE,
      label: String.fromCharCode(Yendor.CHAR_TEEE),
    });
    this.draggable = new Draggable({
      endDragCallback: this.endDragCallback.bind(this),
      maxDragX: (this.options.minWidth || 1) - 1,
      maxDragY: 0,
      minDragX: 0,
      minDragY: 0,
    });
  ...
  public onUpdate(_time: number) {
    if (Draggable.draggedWidget === this.draggable) {
      ...
      (<Widget>this.__parent).local2Abs(pos);
      let maxPos: Core.Position = Umbra.application
        .getConsole()
        .getPixelPositionFromCell(pos.x + this.boundingBox.w, pos.y)!;
      let minPos: Core.Position = Umbra.application
        .getConsole()
      ...
      this.value =
        this.options.minValue +
        ((this.options.maxValue + 1 - this.options.minValue) *
          (pixelPos.x - minPos.x)) /
          (maxPos.x - minPos.x);
      this.value = Core.clamp(
        this.value,
  ...
  private setSliderCharacter() {
    if (this.draggable.getDragPos().x === 0) {
      this.dragButton.getOptions().label = String.fromCharCode(
        Yendor.CHAR_TEEE
      );
...
export class PaddedPanel extends OptionWidget<IPanelOption> {
  ...
  public computeBoundingBox() {
    super.computeBoundingBox();
    this.boundingBox.h += this.options.hPadding || 0;
    this.boundingBox.w += this.options.wPadding || 0;
  }

  public addChild<T extends Core.TreeNode>(node: T): T {
    let ret = super.addChild(node);
    if (node instanceof Umbra.Node) {
  ...
  protected expandChildren(w: number, h: number) {
    for (let child of this.children) {
      (<Widget>child).expand(
        w - (this.options.wPadding || 0) * 2,
        h - (this.options.hPadding || 0) * 2
      );
    }
  }
}
...
export class Frame extends PaddedPanel {
  ...
  constructor(options: IFrameOption) {
    super(options);
    if (!this.options.hPadding || this.options.hPadding < 1) {
      this.options.hPadding = 1;
    }
    if (!this.options.wPadding || this.options.wPadding < 1) {
  ...
  public onRender(console: Yendor.Console) {
    ...
    if (options.footer) {
      console.print(
        Math.floor(pos.x + (this.boundingBox.w - options.footer.length) / 2),
        pos.y + this.boundingBox.h - 1,
        options.footer
      );
    }
  }
  ...
  public computeBoundingBox() {
    ...
    if (this.leftButton) {
      ...
      this.leftButton.moveTo(
        Math.floor(
          this.boundingBox.w / 4 - this.leftButton.getBoundingBox().w / 2
        ),
        this.boundingBox.h - 1
      );
    }
    if (this.rightButton) {
      ...
      this.rightButton.moveTo(
        Math.floor(
          (3 * this.boundingBox.w) / 4 - this.rightButton.getBoundingBox().w / 2
        ),
        this.boundingBox.h - 1
      );
    }
