...
export class ContainerPanel extends Gui.Widget {
  ...
  public initFromContainer(
    wearer: Actors.Actor,
    resolve: (value: Actors.Actor) => void,
  ...
  public onUpdate(_time: number): void {
    if (!Gui.Draggable.isDragging()) {
      this.buildStackedInventory(
        this.rootActor.container,
        this.itemClassFilter
      );
  ...
  private buildStackedInventory(
    container: Actors.Container,
    itemClassFilter?: string
  ) {
    this.inventory = [];
  ...
  private compareActors(a: Actors.Actor[], b: Actors.Actor[]): number {
    let aLevel: number = this.getActorSortLevel(
      a[0],
      ContainerPanel.CATEGORIES
    );
    let bLevel: number = this.getActorSortLevel(
      b[0],
      ContainerPanel.CATEGORIES
    );
  ...
  private createButtons() {
    ...
    let rootDragTarget: Gui.DragTarget = this.itemPanel.addChild(
      new Gui.DragTarget({
        data: this.rootActor,
        dropCallback: this.onDrop.bind(this),
      })
    );
    rootDragTarget.addChild(
      new Gui.Button({
    ...
    for (let itemStack of this.inventory) {
      ...
      if (!item.equipment || !item.equipment.isEquipped()) {
        ...
        if (item.container) {
          draggableContainer = this.itemPanel.addChild(
            new Gui.DragTarget({
              data: item,
              dropCallback: this.onDrop.bind(this),
            })
        ...
        let draggable: Gui.Draggable = draggableContainer.addChild(
          new Gui.Draggable({
            data: itemStack,
          })
        );
  ...
  private onDrop(dropped: Gui.Draggable, _target: Gui.DragTarget, data: any) {
    let droppedActors: Actors.Actor[] = <Actors.Actor[]>(
      dropped.getOptions().data
    );
    let targetActor: Actors.Actor = <Actors.Actor>data;
    if (droppedActors.length > 1) {
      this.numberSelector
        .selectNumber("How many?", 0, droppedActors.length, 1)
        .then((value: number) => {
  ...
  private onDropItem(
    droppedActors: Actors.Actor[],
    targetActor: Actors.Actor,
    count: number
  ) {
    let droppedActor: Actors.Actor = droppedActors[0];
    if (droppedActor.pickable.containerId === targetActor.id) {
      let parentContainer: Actors.Actor | undefined = Actors.Actor.fromId(
        targetActor.pickable.containerId
      );
      if (parentContainer) {
      ...
      if (
        this.putItemsInContainer(
          droppedActors,
          targetActor,
          count,
          targetActor !== this.owner
        ) === 0
      ) {
        if (targetActor === this.owner) {
          Umbra.logger.warn(
            Actors.transformMessage(
              "You cannot take [the actor1].",
              droppedActor
            )
          );
        } else {
          Umbra.logger.warn(
            Actors.transformMessage(
              "You cannot put [the actor1] in [the actor2].",
              droppedActor,
              targetActor
            )
          );
  ...
  private removeItemsFromContainer(
    droppedActors: Actors.Actor[],
    parentContainer: Actors.Actor,
    targetActor: Actors.Actor,
    count: number
  ) {
    ...
    for (let i: number = 0; i < count; ++i) {
      if (
        droppedActors[i].pickable.pick(droppedActors[i], parentContainer, false)
      ) {
        droppedCount++;
      }
    }
    if (droppedCount === 1) {
      ...
      Umbra.EventManager.publishEvent(
        EVENT_CHANGE_STATUS,
        GameStatus.NEXT_TURN
      );
      Umbra.logger.info(
        Actors.transformMessage(
          "You remove [the actor1] from [the actor2].",
          droppedActors[0],
          targetActor
        )
      );
      ...
      Umbra.logger.info(
        Actors.transformMessage(
          "You remove" +
            droppedActors[0].getthename(droppedCount) +
            " from [the actor1].",
          targetActor
        )
      );
  ...
  private putItemsInContainer(
    ...
    for (let i: number = 0; i < count; ++i) {
      if (
        droppedActors[i].pickable.pick(droppedActors[i], targetActor, false)
      ) {
        droppedCount++;
      }
    }
    if (withLog) {
      if (count === 1) {
        ...
        Umbra.logger.info(
          Actors.transformMessage(
            "You put [the actor1] in [the actor2].",
            droppedActors[0],
            targetActor
          )
        );
        ...
        Umbra.logger.info(
          Actors.transformMessage(
            "You put" +
              droppedActors[0].getthename(count) +
              " in [the actor1].",
            targetActor
          )
        );
  ...
  private postRenderItem(
    con: Yendor.Console,
    x: number,
    y: number,
    options: Gui.IButtonOption,
    _active: boolean
  ) {
...
export class SlotsPanel extends Gui.Widget {
  ...
  private createButtons(): boolean {
    ...
    for (let slot of slots) {
      let item: Actors.Actor | undefined = this.owner.container.getFromSlot(
        slot
      );
      if (item) {
        let draggableContainer: Gui.Widget = this.itemPanel;
        ...
        this.itemPanel.addChild(
          new Gui.Button({
            autoHideWidget: this.autoHideWidget,
            label: this.computeEquippedItemLabel(
              slot,
              this.slotWidth,
              undefined,
              1
  ...
  private onDrop(dropped: Gui.Draggable, _target: Gui.DragTarget, data: any) {
    ...
    if (targetActor === this.owner) {
      Umbra.logger.warn(
        Actors.transformMessage("You cannot take [the actor1].", droppedActor)
      );
    } else {
      Umbra.logger.warn(
        Actors.transformMessage(
  ...
  private postRenderEquippedItem(
    con: Yendor.Console,
    x: number,
    y: number,
    options: Gui.IButtonOption,
    ...
    con.fore[x + this.slotWidth][
      y
    ] = Gui.getConfiguration().color.foregroundDisabled;
  }
  ...
  private computeEquippedItemLabel(
    slot: string,
    slotWidth: number,
    item: Actors.Actor | undefined,
    count: number
  ): string {
...
export class SlotContainerPanel extends Gui.Widget {
  ...
  constructor(
    ...
    private frameOption: Gui.IFrameOption = {}
  ) {
    super();
  }
  ...
  public initForCreature(
    ...
    this.containerPanel.initFromContainer(
      wearer,
      resolve,
      autoHideWidget,
      itemClassFilter
  ...
  public onUpdate(time: number) {
    super.onUpdate(time);
    this.setFrameFooter(
      Math.floor(10 * this.owner.container.computeTotalWeight()) / 10 +
        "/" +
...
export class MultiSlotContainerPanel extends SlotContainerPanel {
  ...
  public onInit() {
    super.onInit();
    this.nextItemButton = this.addChild(
      new Gui.Button({
    ...
    this.prevItemButton = this.addChild(
      new Gui.Button({
  ...
  public initForCreatures(
    creatures: Actors.Actor[] | Actors.Actor,
    resolve: (value: Actors.Actor) => void,
    autoHideWidget?: Gui.Widget,
    itemClassFilter?: string
  ) {
  ...
  private setCurrentCreature(index: number) {
    ...
    if (this.creatures.length > 1) {
      let title: string =
        this.creatures[index].getthename() +
        " " +
        (index + 1) +
        "/" +
      ...
      this.nextItemButton.moveTo(
        Math.floor((this.boundingBox.w + title.length) / 2) - 1,
        0
      );
      this.prevItemButton.moveTo(
        Math.floor((this.boundingBox.w - title.length) / 2),
        0
      );
...
export class InventoryPanel
  extends Gui.Widget
  implements Actors.IInventoryItemPicker {
