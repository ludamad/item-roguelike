...
export interface IMapShader {
  ...
  getActorCharCode(
    actor: Actors.Actor,
    renderMode: ActorRenderModeEnum
  ): number;
  getActorColor(
    actor: Actors.Actor,
    renderMode: ActorRenderModeEnum
...
export class BasicMapShader implements IMapShader {
  ...
  public getActorCharCode(
    actor: Actors.Actor,
    _renderMode: ActorRenderModeEnum
  ): number {
    return actor.charCode;
  }

  public getActorColor(
    actor: Actors.Actor,
    _renderMode: ActorRenderModeEnum
  ): Core.Color {
    return actor.col;
...
export class LightDungeonShader implements Umbra.IEventListener, IMapShader {
  ...
  public initForNewMap() {
    ...
    if (player && player.container) {
      for (
        let i: number = 0, count: number = player.container.size();
        i < count;
        ++i
      ) {
        ...
        if (
          item &&
          item.light &&
          (!item.activable || item.activable.isActive())
        ) {
  ...
  public getActorCharCode(
    ...
    if (
      !actor.fovOnly ||
      actor === Actors.Actor.specialActors[Actors.SpecialActorsEnum.PLAYER] ||
    ...
    return Core.ColorUtils.computeIntensity(lightCol) <=
      Actors.PENUMBRA_THRESHOLD
  ...
  public getActorColor(
    ...
    if (
      (!actor.fovOnly && Map.current.isExplored(actor.pos.x, actor.pos.y)) ||
      renderMode === ActorRenderModeEnum.DETECTED
    ) {
    ...
    return Core.ColorUtils.colorMultiply(
      this.nolightShader.getActorColor(actor, renderMode),
      lightCol
    );
  }
  ...
  public getBackgroundColor(map: Map, x: number, y: number): Core.Color {
    ...
    if (map.isWall(x, y) && map.isExplored(x, y)) {
      if (
        ...
        return Core.ColorUtils.colorMultiply(
          this.nolightShader.getBackgroundColor(map, x, y),
          lightCol
        );
      } else {
        return DARK_WALL;
  ...
  public onLightOnoff(actor: Actors.Actor) {
    ...
    if (!actor.activable || actor.activable.isActive()) {
      ...
      let idx: number = this.lights[actor.light.options.renderMode].indexOf(
        actor
      );
      if (idx !== -1) {
