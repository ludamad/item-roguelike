...
export class BspDungeonBuilder extends AbstractDungeonBuilder {
  ...
  protected digMap(map: Map) {
    ...
    bsp.splitRecursive(
      undefined,
      8,
      (<IBspDungeonConfig>this.config).roomMinSize,
      1.5
    );
  ...
  private connectChildren(node: Yendor.BSPNode, map: Map) {
    ...
    let leftPos: Core.Position = this.findFloorTile(
      left.x,
      left.y,
      left.w,
      left.h,
    ...
    let rightPos: Core.Position = this.findFloorTile(
      right.x,
      right.y,
      right.w,
      right.h,
    ...
    let doorPos: Core.Position | undefined = this.findVDoorPosition(
      map,
      leftPos.x,
      leftPos.y,
      rightPos.y
