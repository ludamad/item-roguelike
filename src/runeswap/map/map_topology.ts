...
export class Connector extends TopologyObject {
  ...
  public isDummy(): boolean {
    return this.sector2Id === undefined;
  }
...
export class TopologyMap {
  ...
  public getRandomPositionInSector(
    id: TopologyObjectId,
    rng: Yendor.Random
  ): Core.Position {
   ...
   */
  public computePath(
    id1: TopologyObjectId,
    id2: TopologyObjectId
  ): TopologyObjectId[] {
    let dist: number[] = [];
    let prev: (number | undefined)[] = [];
    let queue: Yendor.BinaryHeap<Sector> = new Yendor.BinaryHeap(
      (o: Sector) => {
        return dist[o.id];
      }
    );
    ...
    while (sector) {
      ...
      for (let connectorId of sector.connectors) {
        ...
        if (connector.lock === undefined) {
          let sector2Id: TopologyObjectId =
            connector.sector1Id === sector.id
              ? connector.sector2Id
              : connector.sector1Id;
   ...
   */
  public findFarthestSector(
    id: TopologyObjectId,
    onlyDeadEnds: boolean = false
  ): TopologyObjectId {
...
export class TopologyAnalyzer {
   ...
   */
  public findDungeonExits(
    entry: Core.Position,
    exit: Core.Position,
    noDeadEndExit: boolean = false
  ): void {
    let longestPathLength: number = 0;
    for (let exitSector of this.topologyMap.sectors) {
      if (noDeadEndExit || exitSector.isDeadEnd()) {
        let farthestSectorId = this.topologyMap.findFarthestSector(
          exitSector.id
        );
        if (farthestSectorId === NONE) {
          continue;
        }
        let pathLength: number = this.topologyMap.computePath(
          exitSector.id,
          farthestSectorId
        ).length;
        if (pathLength > longestPathLength) {
          longestPathLength = pathLength;
          let entrySector: Sector = this.topologyMap.getSector(
            farthestSectorId
          );
   ...
   */
  public buildPuzzle(
    entry: TopologyObjectId,
    exit: TopologyObjectId,
    keyNumber: number = 0
  ) {
    ...
    do {
      ...
      for (let connectorId of currentSector.connectors) {
        ...
        if (
          connector.sector1Id === nextSector.id ||
          connector.sector2Id === nextSector.id
        ) {
      ...
      (lastConnector === undefined || !lastConnector.gut)
    );
    if (lastConnector && lastConnector.gut) {
      ...
      if (keySectorId !== NONE) {
        ...
        this.topologyMap.puzzle.push({
          connectorId: lastConnector.id,
          keySectorId: keySectorId,
        });
  ...
  private floodFill(map: Map, x: number, y: number) {
    ...
    while (cellsToVisit.length !== 0) {
      ...
      for (let curpos of adjacentCells) {
        if (
          map.isWall(curpos.x, curpos.y) ||
          this.topologyMap.getObjectId(curpos) === sector.id
        ) {
          continue;
        }
        if (this.topologyMap.getObjectId(curpos) === NONE) {
          if (map.canWalk(curpos.x, curpos.y)) {
            ...
            (curpos.x === pos.x || curpos.y === pos.y) &&
            this.hasDoor(map, curpos)
          ) {
            // a new connector
            this.topologyMap.setObjectId(
              curpos,
              this.newConnector(pos, curpos, sector.id)
            );
          }
          ...
          if (
            connector.sector1Id !== sector.id &&
            connector.sector2Id === undefined
          ) {
            connector.sector2Id = sector.id;
            this.topologyMap
              .getSector(connector.sector1Id)
              .addConnector(connector.id);
            this.topologyMap
              .getSector(connector.sector2Id)
              .addConnector(connector.id);
          }
        }
      }
    ...
    while (toExplore.length > 0) {
      ...
      for (let connectorId of sector.connectors) {
        ...
        if (connector2.id !== connector.id) {
          if (
            connector2.sector1Id === sector.id &&
            !connector2.isDummy() &&
            explored.indexOf(connector2.sector2Id) === -1
          ) {
            toExplore.push(connector2.sector2Id);
          } else if (
            connector2.sector2Id === sector.id &&
            explored.indexOf(connector2.sector1Id) === -1
          ) {
