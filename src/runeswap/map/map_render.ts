...
export abstract class MapRendererNode extends Umbra.Node {
  ...
  public abstract getActorRenderMode(
    actor: Actors.Actor,
    detectRange: number
  ): ActorRenderModeEnum;
  ...
  protected abstract renderActor(
    con: Yendor.Console,
    actor: Actors.Actor,
    detectRange: number
  ): void;
...
export class DungeonRendererNode extends MapRendererNode {
  ...
  public canIdentifyActor(actor: Actors.Actor): boolean {
    return this.__renderMode === MapRenderModeEnum.ALL_SEEING_EYE
      ? true
      : this.shader.canIdentifyActor(actor);
  }

  public getCellLightLevel(x: number, y: number): CellLightLevelEnum {
    return this.__renderMode === MapRenderModeEnum.ALL_SEEING_EYE
      ? CellLightLevelEnum.LIGHT
      : this.shader.getCellLightLevel(x, y);
  }
  ...
  protected renderActor(
    ...
    let renderMode: ActorRenderModeEnum = this.getActorRenderMode(
      actor,
      detectRange
    );
    if (renderMode !== ActorRenderModeEnum.NONE) {
      let pos: Core.Position = actor.pos;
      con.text[pos.x][pos.y] =
        this.__renderMode === MapRenderModeEnum.ALL_SEEING_EYE
          ? actor.charCode
          : this.shader.getActorCharCode(actor, renderMode);
      con.fore[pos.x][pos.y] =
        this.__renderMode === MapRenderModeEnum.ALL_SEEING_EYE
          ? actor.col
