...
export class NumberSelector
  extends Gui.Widget
  implements Actors.INumberSelector {
  ...
  public onInit() {
    ...
    this.label = vpanel.addChild(
      new Gui.Label<Gui.ILabelOption>({ label: "0" })
    );
  ...
  public onUpdate(time: number) {
    super.onUpdate(time);
    this.label.getOptions().label = Math.floor(this.slider.getValue()).toFixed(
      0
    );
  }
  ...
  public selectNumber(
    ...
    return new Promise<number>((resolve) => {
      ...
      this.slider.setValue(
        initialValue === undefined ? minValue : initialValue
      );
      this.show();
    });
  }
